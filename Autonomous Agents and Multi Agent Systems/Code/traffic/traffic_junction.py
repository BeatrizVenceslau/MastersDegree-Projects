...
from traffic.utils.action_space import MultiAgentActionSpace
from traffic.utils.draw import draw_grid, draw_circle, fill_cell, write_cell_text
from traffic.utils.observation_space import MultiAgentObservationSpace
...
class TrafficJunction(gym.Env):
    ...
    def __init__(self, grid_shape=(14, 14), step_cost=-0.01, n_max=4, collision_reward=-10, arrive_prob=0.5,
        ...
        self.agent_collision = [False for _ in range(n_max)]
    ...
    def step(self, agents_action):
        ...
        self.agent_collision = [False for _ in range(self.n_agents)]
        ...
        for agent_i, action in enumerate(agents_action):
            if not self._agent_dones[agent_i] and self._on_the_road[agent_i]:
                ...
                if collision_flag:
                    ...
                    self.agent_collision[agent_i] = True
                    step_collisions += 1
    ...
    def render(self, mode: str = 'human'):
        ...
        for agent_i in range(self.n_agents):
            if not self._agent_dones[agent_i] and self._on_the_road[agent_i]:
                ...
                if self.agent_collision[agent_i]:
                    draw_circle(img, self.agent_pos[agent_i], cell_size=CELL_SIZE, fill="white", radius=0.1)
