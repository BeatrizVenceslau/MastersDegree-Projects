...
class FeedforwardNetwork(nn.Module):
    def __init__(
        ...
        self.activation = nn.Tanh if activation_type == "tanh" else nn.ReLU
        layers_l = [
            nn.Flatten(),
            nn.Dropout(dropout),
            self.activation(),
        ...
        layers_l += [nn.Linear(hidden_size, hidden_size), nn.Dropout(dropout), self.activation()] * (layers-1)
        layers_l.append(nn.Linear(hidden_size, n_classes))
