...
class Decoder(nn.Module):
    ...
    def forward(self, word, decoder_hidden_state, decoder_cell_state, encoder_out=None):
        ...
        # print(word)
        # print(type(word))
        # print(type(decoder_hidden_state))
        # print(type(decoder_cell_state))

        """
        Decode_step(input, [h_0, c_0])
        Args:
            input - tensor containing input features
            h_0 - tensor containing the initial hidden state for each element in the batch, def = 0
            c_0 - tensor containing the initial cell state for each element in the batch, def = 0
        """
